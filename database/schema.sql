...
CREATE TABLE IF NOT EXISTS payment_reminder_deliveries (
  ...
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  KEY payment_reminder_user_created_idx (user_id, created_at),
  KEY payment_reminder_status_created_idx (status, created_at),
  CONSTRAINT payment_reminder_user_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
