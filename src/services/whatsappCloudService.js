...
async function sendBoletoWhatsappTemplate({ recipient, customerName, amount, dueDate, paymentLink }, options = {}) {
    ...
    `https://graph.facebook.com/${encodeURIComponent(config.apiVersion)}/${encodeURIComponent(config.phoneNumberId)}/messages`,
    {
      ...
      body: JSON.stringify({
        ...
        template: {
          ...
          components: [{
            type: "body",
            parameters: [customerName, amount, dueDate, paymentLink].map((text) => ({ type: "text", text: String(text || "-") })),
          }],
        },
      }),
  ...
  if (!response.ok) {
    const error = new Error(data.error?.message || "Falha ao enviar boleto pelo WhatsApp.");
...
export { getWhatsappCloudConfig, isWhatsappCloudConfigured, normalizeWhatsappRecipient, sendBoletoWhatsappTemplate };
